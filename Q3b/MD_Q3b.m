% This file is the code for ELEC 4700 assignment 1 question 3b: injection
% Electron Modeling
...
% Loop for simulation
for iSim = 1:numSim
    % Check for create electron
    if mod(iSim, deltaSimCreateE) == 0
        % Introduce an electron
        x(iCreateE) = 0;
        vx(iCreateE) = abs(sqrt(C.kb*T/C.mn).*randn());  % Positive vx
        vy(iCreateE) = sqrt(C.kb*T/C.mn).*randn();
        iCreateE = iCreateE+1;
    end 
    ...
    for iE=1:numE
        ...
        if bScattering && ~bInvalid && Pscat > rand() && x(iE) ~= -10
            % Rethermalize
    ...
    vth2_mean = mean(sqrt(vx(1:(iCreateE-1)).^2+vy(1:(iCreateE-1)).^2)).^2;
...
tempDisplay(numGridX, numGridY, iCreateE, Region.x, Region.y);
